...
Optional for Fast Path and Lightweight when there is **no durable context change**. Required for Full packets when memory files were updated. This PR template keeps the field visible for review. For Fast Path and Lightweight changes with no durable context change, fill `no durable context change`; ordinary non-PR delivery may omit Memory judgment as allowed by `AGENTS.md`.
