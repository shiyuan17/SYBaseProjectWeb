...
export class TrackingExceptionPage {
  ...
  async openApplicationTracking(applicationNo: string) {
    ...
    const row = this.page.locator('.el-table__row').filter({ hasText: applicationNo }).first();
    ...
    return (await detailResponse.json()).data as TrackingPayload;
  }
  ...
  async openSpecimenTracking(barcode: string) {
    ...
    const row = this.page.locator('.el-table__row').filter({ hasText: barcode }).first();
  ...
  async assertHappyPath(payload: TrackingPayload, barcodes: string[]) {
    ...
    for (const barcode of barcodes) {
      expect(payload.specimens.some((item) => item.barcode === barcode)).toBeTruthy();
  ...
  async assertAbnormalPath(
    ...
    const abnormalSpecimen = payload.specimens.find((item) => item.barcode === abnormalBarcode);
    ...
    expect(abnormalSpecimen?.qualityIssueCodes?.includes('CONTAINER_DAMAGE')).toBeTruthy();
