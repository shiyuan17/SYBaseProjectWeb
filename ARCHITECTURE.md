...
- M6 statistics lives under `apps/web-ele/src/modules/m6-statistics` and consumes sibling backend `../SYBaseProject/bl-center` `/api/v1/stat-*` APIs. `/m6/dashboard` uses `POST /api/v1/stat-dashboard/query` and dashboard cards grouped as summary/quality/operation/workload; `/m6/quality-indicators` uses `POST /api/v1/stat-reports/query` with `category=QUALITY` and displays availability/source metadata instead of placeholder content. Backend-menu mapping recognizes `M6_SUPPORT` root plus `M6_DASHBOARD` child. Statistic report query filters use `workloadUserId`; report rows may include optional metric metadata and availability status, but legacy sparse rows remain valid. CSV export for reports stays UTF-8 BOM CSV with `indicatorCode,indicatorName,metricValue,metricUnit,metricStatus,numerator,denominator,sourceNote`.
