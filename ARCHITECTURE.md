...
- M5 archive object lists use sibling backend `../SYBaseProject/bl-center` `GET /api/v1/archive-objects` with `objectType=APPLICATION_FORM|EMBEDDING_BOX|SLIDE`, `keyword`, `page`, and `size` to return all queryable application forms, wax blocks, and slides plus archive/loan status. Frontend `/operation-support/archive` must keep each object tab's pagination/filter state independent and keep `GET /api/v1/archive-records/search` for archive-record query consumers.
