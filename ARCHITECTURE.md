...
- M6 statistics lives under `apps/web-ele/src/modules/m6-statistics` and consumes sibling backend `../SYBaseProject/bl-center` `/api/v1/stat-*` APIs. Statistic report query filters use `workloadUserId`; report rows may include optional metric metadata and availability status, but legacy sparse rows remain valid. CSV export for reports stays UTF-8 BOM CSV with `indicatorCode,indicatorName,metricValue,metricUnit`.
