...
8. 父任务必须写 `Execution Mode: orchestrator` 和 `Run Policy: single-child`；子任务必须写 `Execution Mode: goal`。子任务 ID 使用 `<TASK_ID>.<NNN>`，只写入父任务目录 `task.json`，不写入全局 `backlog.json`。
9. 若任务来源平台有现成记录能力，可在输出中标注建议的标题、编号和状态同步方式；不强绑定任何平台。

## 拆分要求

- 每个任务项必须有目标、验收标准、非目标、影响范围、Dynamic Workflow、Memory、Evidence、停止条件和验证要求。
- 目录化父任务必须有 `Execution Mode: orchestrator`、`Run Policy`、`Parent Completion Check`；默认 `single-child`，仅明确需要连续推进时使用 `batch` 且 `maxChildrenPerRun` 为 1 到 3。
- 目录化子任务必须有 `Execution Mode: goal`、`Timebox: <= 5 minutes`、`Stop Condition`、`Verification Command`、`Rollback Plan`、`Non-goals`、`Evidence`。
...
6. 父任务 Orchestrator Goal 提示词和每个子任务 Goal 提示词；父任务提示词只允许编排，子任务提示词必须只引用单个 `<TASK_ID>.<NNN>`。
