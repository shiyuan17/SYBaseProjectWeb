...
| 问题 | 默认动作 |
| --- | --- |
| 当前工作区是否脏？ | 先 `git status --short`，只处理本任务路径；归属不清则暂停 |
| 是否必须使用 worktree？ | 命中运行时代码 + 共享契约/构建/跨仓/多提交/无法隔离脏改动时使用；低风险文档或只读可例外 |
| 能否提交？ | 低/中风险按 §3.1；高风险、红区或用户要求等待时不提交 |
| PR 需要哪些 Packet？ | 按 `DYNAMIC_WORKFLOW_RULES.md` 选择 Fast Path / Lightweight / Full |

Tag 与发布只在显式发布动作中处理；版本 tag、发布 tag 闭环和回滚以 `RELEASE.md` 为准。
...
```markdown
Commit group: Included files: Excluded files: Validation: Risk: Rollback: Needs human approval?:
```
...
发布版本、版本号与发布说明的闭环以 `RELEASE.md` 为唯一来源；本节只定义 Git tag 命名与禁止项。
...
- Review 五轴、严重度、阻塞条件、AI checker 和证据质量以 `REVIEW_RULES.md` 为唯一细则来源
...
> 注意：`pr-packet.yml` 只校验 PR 正文存在关键 Dynamic Workflow / Memory / Evidence 字段且非空；当 PR 声明启用了 Red Team 时，还会检查 `Attack result` 与 `Residual risk` 不为空，但仍不判断证据质量。`governance-quality.yml` 只在治理/记忆/模板路径变更时运行，负责 `check:governance` 与治理脚本测试。`cspell` 扫描 `**/*.{ts,tsx,vue}`、`**/README.md`、`docs/**/*.md` 与根目录 `*.md`（含五类记忆文件），新增领域术语需在 `cspell.json` 词典登记；章节一致性等非拼写问题仍须人工核对。Red Team 质量、人工确认是否真实完成等治理项仍依赖交付者和 reviewer 审查。

### 8. 动态 Workflow 剧本与红队审查

PR 必须根据任务类型填写 Dynamic Workflow / Memory / Evidence。主 Workflow 选择、强制修饰器、专家 Agent、动态测试/模拟与 Red Team 要求以 `docs/rules/DYNAMIC_WORKFLOW_RULES.md` 为唯一来源，本文件不再复述各 Workflow 的触发条件与剧本；不得所有任务套用同一套 AI 流程。

PR 合入前还必须填写 Memory：

- 记忆文件的职责与更新触发条件以 `AGENTS.md` 第 8 节「AI Memory Update」为唯一来源，交付前按其规则按需更新 `docs/memory/` 五类记忆文件
- PR 必须说明已更新文件、未更新原因、相关记忆项 ID、跨仓引用和剩余风险；无 durable context 变更时写 `no durable context change`
- `pr-packet.yml` 会拦截缺少关键字段或字段为空的 PR；Dynamic Workflow、Memory 和 Evidence 的内容质量仍由 reviewer 结合证据审查
...
- [ ] PR 描述包含目的、影响、验证和风险
- [ ] PR 已填写 Dynamic Workflow / Memory / Evidence，高风险变更已执行 Red Team
- [ ] 使用 Loop Engineering 执行的任务已填写 Loop Packet
- [ ] Memory 已引用相关记忆项 ID 或说明 `no durable context change`
