...
若任务通过 Loop Engineering 方式执行，还必须填写 `docs/LOOP_ENGINEERING_RULES.md` 定义的 Loop Packet；低风险任务可使用该文件定义的「最小 Loop Packet」，无需为形式合规补全字段。Loop Packet 只描述本轮闭环如何运行、何时停止、状态写到哪里；不替代本文的 Workflow 分类、修饰器、动态测试、Red Team 或 Memory Update 要求。
...
下图把「速查表选主 Workflow → 叠修饰器 → worktree 决策 → 红区升级」串成一条可执行路径；语义以本文「总规则」与速查表为准，worktree 判定以 `docs/GIT_RULES.md` 第 6 节为准，Fast Path 模板见 `AGENTS.md` 第 4 节：
