...
async function loadReportContext() {
  ...
  if (!caseId.value) {
    pageError.value = '缺少病例 ID，无法加载报告上下文。';
    return;
  }
  ...
  pageError.value = '';
  try {
    const workbench = await getDiagnosticWorkbench(caseId.value);
...
<template>
  <Page title="报告编辑与流转" description="创建草稿、保存正文，并完成提交、审核、驳回、签发、发布闭环。">
    <div class="flex flex-col gap-4">
      ...
      <WorkflowSectionCard title="流转操作" description="操作人姓名为后端必填字段，驳回时需填写驳回原因。">
        <ElForm label-width="100px">
          ...
          <ElFormItem>
            <div class="flex flex-wrap gap-2">
              <ElButton :loading="saving" type="primary" @click="createDraft">
                创建草稿
              </ElButton>
              <ElButton :loading="saving" type="success" @click="saveDraft">
                保存草稿
              </ElButton>
              <ElButton :loading="saving" @click="runReportAction('submit')">
                提交
              </ElButton>
              <ElButton :loading="saving" @click="runReportAction('review')">
                审核通过
              </ElButton>
              <ElButton :loading="saving" type="danger" @click="rejectReport">
                驳回
              </ElButton>
              <ElButton :loading="saving" @click="runReportAction('sign')">
                签发
              </ElButton>
              <ElButton :loading="saving" @click="runReportAction('publish')">
                发布
              </ElButton>
            </div>
          </ElFormItem>
