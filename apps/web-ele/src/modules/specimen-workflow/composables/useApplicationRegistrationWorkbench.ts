...
} from '../utils/application-registration-auto-create';
...
export function useApplicationRegistrationWorkbench(
  ...
  async function hydrateWorkbenchRecord(
    record: ApplicationRegistrationWorkbenchRecord,
  ) {
  ...
  async function createApplicationWorkbenchRecord(applicationNo: string) {
    if (isApplicationNoTooLong(applicationNo)) {
      throw new Error('申请单号不能超过 64 个字符，请核对后再查询。');
    }

    let createdApplication;
    try {
      createdApplication = await createApplication(
        buildAutoCreatedApplicationPayload(applicationNo),
      );
    } catch (error) {
      const status = getResponseStatus(error);
      if (status === 403) {
        throw new Error('未找到申请单，且当前账号没有申请单创建权限。', {
          cause: error,
        });
      }
      if (status === 409) {
        return lookupApplicationRegistrationWorkbenchRecord({
  ...
  async function loadWorkbench(
    ...
    if (!record) {
      if (lookupType === 'APPLICATION_NO') {
        const createdRecord = await createApplicationWorkbenchRecord(keyword);
        if (createdRecord) {
          return hydrateWorkbenchRecord(createdRecord);
        }
      }
      clearWorkbenchState(getLookupNotFoundDescription(lookupType));
      return null;
    }

    return hydrateWorkbenchRecord(record);
  }
