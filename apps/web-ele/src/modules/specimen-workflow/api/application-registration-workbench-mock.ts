...
import { buildAutoGeneratedWorkbenchPatientInfo } from '../utils/application-registration-auto-create';
import {
  buildSearchKeywords,
...
export function resetApplicationRegistrationWorkbenchMockState() {
  workbenchRecords = rawRecords.map((record) => mapRecord(record));
}
...
export function createAutoGeneratedWorkbenchRecordMock(
  created: ApplicationCreateResult,
  data: ApplicationCreateRequest,
) {
  const normalizedApplicationNo = data.applicationNo?.trim();
  if (
    !normalizedApplicationNo ||
    workbenchRecords.some(
      (record) => record.patientInfo.applicationNo === normalizedApplicationNo,
    )
  ) {
    return;
  }

  const payload = buildAutoGeneratedWorkbenchPatientInfo(
    null,
    created,
    normalizedApplicationNo,
  );
