...
vi.mock('../api/application-registration-workbench-service', () => ({
  getSpecimenDictionary: getSpecimenDictionaryMock,
          ...
          h(
            ...
            props.record
              ? [
                  ...
                  props.record.patientInfo.clinicalDiagnosis,
...
function createAutoGeneratedRecordFixture(
  applicationNo = 'AUTO-APP-001',
): ApplicationRegistrationWorkbenchRecord {
  return {
    ...createRecordFixture(),
    applicationId: 'app-auto-created',
    contagiousSpecimen: {
      hepatitis: false,
      hiv: false,
      isolation: true,
    ...
    gynecologyInfo: {
      additionalNotes: '自动生成申请单，妇科专项信息按常规阴性模板生成。',
      hpvResult: '阴性',
      lastMenstrualPeriod: '2026-05-08',
      menopause: false,
      previousCytology: '既往检查未见异常',
      previousTreatment: '无',
      specialConditions: {
        abnormalBleeding: false,
        birthControl: false,
    ...
    specimenItems: [],
    surgeryInfo: {
      ...createRecordFixture().surgeryInfo,
      buildingId: 'B001',
      clinicalFindings: '胫骨近端骨质破坏，周围软组织充血水肿。',
      fixationPerson: '赵护士',
      fixationTime: '2026-06-08 10:20:15',
      roomId: 'OR-102',
      specimenRemovalTime: '2026-06-08 10:07:02',
...
function resetMocks() {
  ...
  getSpecimenDictionaryMock.mockReset();
  ...
  getSpecimenDictionaryMock.mockResolvedValue({
    commonOptions: commonSpecimenOptionsFixture,
    departmentFiltered: true,
    entryOptions: specimenEntryOptionsFixture,
    groups: dictionaryGroupsFixture,
  });
...
describe('ApplicationRegistrationWorkbenchPanel', () => {
  ...
  it('auto-creates on the first AUTO-triggered lookup without leaving an error alert', async () => {
    const autoRecord = createAutoGeneratedRecordFixture('AUTO-TRIGGER-001');
    ...
    const wrapper = await mountPanel({
      lookupKeyword: 'AUTO-TRIGGER-001',
      lookupQueryType: 'AUTO',
      lookupTriggerKey: 1,
    });
    await wrapper.wait();
    ...
    expect(
      wrapper.root.querySelector('[data-testid="patient-panel-text"]')
        ?.textContent,
    ).toContain('林晓芸|AUTO-TRIGGER-001|慢性骨髓炎');

    wrapper.unmount();
  });

  it('auto-creates an application when application number lookup misses', async () => {
    const autoRecord = createAutoGeneratedRecordFixture('AUTO-APP-001');
    lookupApplicationRegistrationWorkbenchRecordMock.mockResolvedValueOnce(
      null,
    );
    saveApplicationRegistrationPatientInfoMock.mockResolvedValueOnce(
    ...
    expect(createApplicationMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        patientAge: expect.stringMatching(/^\d+$/),
        patientGender: expect.stringMatching(/男|女/),
        patientId: expect.stringMatching(/^\d{5}$/),
    ...
    expect(saveApplicationRegistrationPatientInfoMock).toHaveBeenCalledWith(
      'app-auto-created',
      expect.objectContaining({
        contagiousSpecimen: expect.objectContaining({
        ...
        gynecologyInfo: expect.objectContaining({
          ...
          specialConditions: expect.objectContaining({
    ...
    expect(
      saveApplicationRegistrationPatientInfoMock.mock.calls[0]?.[1],
    ).not.toHaveProperty('specimenItems');
    expect(
      wrapper.root.querySelector('[data-testid="patient-panel-text"]')
        ?.textContent,
    ).toContain('林晓芸|AUTO-APP-001|慢性骨髓炎');
    expect(
      wrapper.root.querySelector('[data-testid="specimen-items"]')?.textContent,
    ).toBe('[]');
    expect(
  ...
  it('does not auto-create when non-application-number lookup misses', async () => {
    lookupApplicationRegistrationWorkbenchRecordMock.mockResolvedValueOnce(
      null,
    );
    const wrapper = await mountPanel();
    ...
    select!.value = 'PATIENT_NAME';
    select!.dispatchEvent(new Event('change'));
    await wrapper.wait();

    wrapper.search('不存在患者');
    await wrapper.wait();
  ...
  it('shows a contextual permission error when auto-create is forbidden', async () => {
    lookupApplicationRegistrationWorkbenchRecordMock.mockResolvedValueOnce(
      null,
    );
    createApplicationMock.mockRejectedValueOnce({
      response: {
        status: 403,
      },
    });
    ...
    wrapper.search('AUTO-FORBIDDEN');
    await wrapper.wait();

    expect(createApplicationMock).toHaveBeenCalledTimes(1);
    expect(
      wrapper.root.querySelector('[data-testid="alert"]')?.textContent,
    ).toContain('未找到申请单，且当前账号没有申请单创建权限。');
    expect(
      wrapper.root.querySelector('[data-testid="patient-panel"]'),
    ).toBeNull();
  ...
  it('retries lookup when auto-create reports an application conflict', async () => {
    const conflictRecord = createAutoGeneratedRecordFixture('AUTO-CONFLICT');
    lookupApplicationRegistrationWorkbenchRecordMock
      .mockResolvedValueOnce(null)
      .mockResolvedValueOnce(conflictRecord);
    createApplicationMock.mockRejectedValueOnce({
      response: {
        status: 409,
      },
    ...
    wrapper.search('AUTO-CONFLICT');
    await wrapper.wait();

    expect(createApplicationMock).toHaveBeenCalledTimes(1);
    expect(saveApplicationRegistrationPatientInfoMock).not.toHaveBeenCalled();
    expect(
      lookupApplicationRegistrationWorkbenchRecordMock,
    ).toHaveBeenLastCalledWith({
      keyword: 'AUTO-CONFLICT',
      queryType: 'APPLICATION_NO',
    });
    expect(
      wrapper.root.querySelector('[data-testid="patient-panel-text"]')
        ?.textContent,
    ).toContain('AUTO-CONFLICT');
